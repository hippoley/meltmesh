...
window.createThreeRenderer = async function createThreeRenderer(canvas, getState) {
  ...
  const sdfFragment = source.fragmentSource
    ...
    .replace('  col=1.0-exp', '  vec2 screenUv=gl_FragCoord.xy/uResolution;float sceneDepth=texture(uSceneDepth,screenUv).r;if(id<=0.0){gl_FragDepth=sceneDepth;fragColor=texture(uSceneColor,screenUv);return;}vec3 fieldHit=ro+rd*t;vec4 clipPosition=uViewProjection*vec4(fieldHit,1.0);float sdfDepth=clamp(clipPosition.z/clipPosition.w*0.5+0.5,0.0,1.0);float meshOwnership=uHasMeshSdf==1?importedMaterialWeight(fieldHit):0.0;bool originalMaterialOwnsPixel=meshOwnership>0.68&&sceneDepth<=sdfDepth+0.0035;bool groundOwnsPixel=id>1.5&&sceneDepth<=sdfDepth+0.0035;if(sceneDepth<sdfDepth-0.0002||originalMaterialOwnsPixel||groundOwnsPixel){gl_FragDepth=sceneDepth;fragColor=texture(uSceneColor,screenUv);return;}gl_FragDepth=sdfDepth;\n  col=1.0-exp')
...
if (uInteractionEnabled > 0.5) {
  ...
  float penetration = 1.0 - smoothstep(-band * 0.72, band * 0.10, contactDistance);
...
#include <output_fragment>`);
  ...
  function frame() {
    ...
    sdfUniforms.uConsumeScale.value=state.consumeScale;sdfUniforms.uBooleanSmooth.value=state.booleanSmooth;sdfUniforms.uFrontNoise.value=state.frontNoise;
