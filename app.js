...
function rebuildImportedVolume(){
  ...
  for(let z=0;z<resolution;z++)for(let y=0;y<resolution;y++)for(let x=0;x<resolution;x++){
    ...
    const k=Math.max(state.blend*.72,.025),h=Number.isFinite(second)?Math.max(k-Math.abs(best-second),0)/k:0;sdf[index]=Number.isFinite(second)?Math.min(best,second)-h*h*k*.25:best;const a=winner?.materialData?.subarray(winnerIndex*4,winnerIndex*4+4)||[143,158,173,72],b=runnerUp?.materialData?.subarray(runnerIndex*4,runnerIndex*4+4)||a,weight=h*.5;for(let channel=0;channel<4;channel++)material[index*4+channel]=Math.round(a[channel]*(1-weight)+b[channel]*weight);
  }
