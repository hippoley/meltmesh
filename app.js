...
void main(){
  ...
  if(id>0.0){
    ...
    if(id<1.5&&uHasMeshSdf==1&&uHasMeshMaterial==1){float materialWeight=importedMaterialWeight(p);vec3 materialUv=clamp(importedUv(p),0.0,1.0);vec4 imported=texture(uMeshMaterial,materialUv);vec4 features=uHasMeshFeatures==1?texture(uMeshFeatures,materialUv):vec4(0.0,1.0,0.0,0.0);float junction=pow(max(0.0,1.0-abs(materialWeight*2.0-1.0)),1.7),ownership=smoothstep(0.04,0.96,materialWeight);albedo=mix(albedo,imported.rgb,ownership);albedo=mix(albedo,normalize(max(albedo,vec3(0.001)))*1.08,junction*0.24);surfaceRoughness=mix(surfaceRoughness,imported.a,smoothstep(0.08,0.92,materialWeight));surfaceRoughness=mix(surfaceRoughness,0.12,junction*0.34);surfaceMetalness=mix(surfaceMetalness,features.r,ownership);surfaceAlpha=mix(surfaceAlpha,features.g,ownership);surfaceEmission=mix(surfaceEmission,features.b,ownership);surfaceTransmission=mix(surfaceTransmission,features.a,ownership);}
    ...
    else{float fresnel=pow(1.0-noV,5.0),reflectionWeight=clamp(mix(0.08,0.92,fresnel)*uSpecular+surfaceMetalness*0.34,0.0,1.0);vec3 reflection=sky(reflect(rd,n));vec3 refraction=sky(refract(rd,n,0.66))*mix(vec3(1.0),albedo,0.18);float highlight=pow(noH,mix(220.0,48.0,surfaceRoughness))*shadow;vec3 dielectric=mix(refraction,reflection,reflectionWeight),metal=reflection*mix(vec3(0.72),albedo,0.72);col=mix(dielectric,metal,surfaceMetalness)+highlight*vec3(1.0,0.98,0.94)*2.4+albedo*(0.035+surfaceEmission*0.42);col=mix(col,refraction,clamp(surfaceTransmission*(1.0-surfaceMetalness)*0.28,0.0,0.7));col=mix(col,col*0.62+reflection*0.38,reaction*0.56);col*=mix(0.72,1.0,surfaceAlpha);}
