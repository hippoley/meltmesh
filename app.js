...
function rebuildImportedVolume(){
  ...
  webgpuRenderer?.setVolume(sdf,material,resolution,features);threeRenderer?.setVolume?.(sdf,material,resolution,features);state.meshVolumeReady=true;document.getElementById('fusionLabel').textContent=`统一 SDF + ${sources.length} 个源材质场`;
}
...
if(window.createWebGpuRenderer){window.createWebGpuRenderer(document.getElementById('gpuCanvas'),()=>state).then(renderer=>{if(!renderer){document.getElementById('renderStatus').textContent='WebGL2 · 无 WebGPU 适配器';return;}if(window.unifiedRendererActive){renderer.stop();return;}webgpuRenderer=renderer;if(meshSdfData&&meshMaterialData)renderer.setVolume(meshSdfData,meshMaterialData,Math.round(Math.cbrt(meshSdfData.length)),meshFeaturesData);document.getElementById('viewport').classList.add('webgpu-active');document.getElementById('renderStatus').textContent=threeModelReady?'Three.js PBR + WebGPU SDF':'WebGPU 实时';}).catch(error=>{console.warn('WebGPU fallback:',error);const message=String(error?.message||error),status=document.getElementById('renderStatus');status.textContent=`WebGL2 · ${message.slice(0,72)}`;status.title=message;showToast(`WebGPU：${message}`,true);fetch(`/client-error?source=webgpu&message=${encodeURIComponent(message)}`).catch(()=>{});});}
