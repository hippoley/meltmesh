...
vec3 reflectionField(vec3 p,vec3 n,float reaction,float impedance){float spectrum=0.5+0.5*refractiveSpectrum(p,n);float cells=fieldNoise(p*18.0+n*2.7+vec3(uTime*0.03));float veins=smoothstep(0.48,0.72,abs(quasiPattern(p*3.4+n*1.8)));vec3 cyan=vec3(0.02,0.95,0.88),amber=vec3(1.0,0.44,0.08),violet=vec3(0.48,0.38,1.0);vec3 band=mix(cyan,amber,spectrum);band=mix(band,violet,veins*0.42);float crystal=pow(clamp(spectrum*0.62+cells*0.38,0.0,1.0),1.7);return band*(0.18+1.65*reaction*(0.35+0.65*crystal)*(0.75+impedance));}
...
void main(){
  ...
  if(id>0.0){
    ...
    vec3 albedo=uColor;float surfaceRoughness=uRoughness,surfaceMetalness=0.0,surfaceAlpha=1.0,surfaceEmission=0.0,surfaceTransmission=uTransmission;float reaction=id<1.5?fusionReaction(p,n):0.0;float spectrum=0.5+0.5*refractiveSpectrum(p,n);
    if(id<1.5){vec4 residue=mix(uBoxResidue,uSphereResidue,surfaceIsSphere);vec3 residueColor=mix(uBoxResidueColor,uSphereResidueColor,surfaceIsSphere);float residueMask=clamp(residue.x*(0.55+0.45*spectrum),0.0,1.0);albedo=mix(albedo,residueColor,residueMask*0.46);surfaceTransmission=max(surfaceTransmission,residue.z*0.42);surfaceEmission=max(surfaceEmission,residue.y*0.35);surfaceRoughness=mix(surfaceRoughness,0.018,residue.w*0.28);}
    if(id<1.5&&uHasMeshSdf==1&&uHasMeshMaterial==1){float materialWeight=importedMaterialWeight(p);vec3 materialUv=clamp(importedUv(p),0.0,1.0);vec4 imported=texture(uMeshMaterial,materialUv);vec4 features=uHasMeshFeatures==1?texture(uMeshFeatures,materialUv):vec4(0.0,1.0,0.0,0.0);float junction=pow(max(0.0,1.0-abs(materialWeight*2.0-1.0)),1.35),ownership=smoothstep(0.04,0.96,materialWeight);float impedance=clamp(length(imported.rgb-uColor)*0.95+abs(features.r-uRoughness)*0.55+features.a*0.52,0.0,1.0);vec3 refractedCopy=mix(imported.rgb,reflectionField(p,n,max(reaction,junction*0.68),impedance),clamp(junction*0.56+reaction*0.34,0.0,0.86));albedo=mix(albedo,imported.rgb,ownership);albedo=mix(albedo,refractedCopy,clamp(junction*0.44+reaction*0.38,0.0,0.92));surfaceRoughness=mix(surfaceRoughness,imported.a,smoothstep(0.08,0.92,materialWeight));surfaceRoughness=mix(surfaceRoughness,0.015,junction*0.46+reaction*0.38);surfaceMetalness=mix(surfaceMetalness,max(features.r,junction*0.32),ownership);surfaceAlpha=mix(surfaceAlpha,features.g,ownership);surfaceEmission=mix(surfaceEmission,max(features.b,junction*0.35),ownership);surfaceTransmission=mix(surfaceTransmission,max(features.a,junction*0.72),ownership);}
    if(id<1.5){float impedance=clamp(surfaceMetalness*0.62+(1.0-surfaceRoughness)*0.24+surfaceTransmission*0.32,0.0,1.0);vec3 spectralTint=reflectionField(p,n,reaction,impedance);albedo=mix(albedo,albedo*vec3(0.58,0.82,0.78),reaction*0.24);albedo=mix(albedo,spectralTint,reaction*0.36);surfaceRoughness=mix(surfaceRoughness,0.012,reaction*0.84);surfaceMetalness=max(surfaceMetalness,reaction*0.22);surfaceEmission=max(surfaceEmission,reaction*0.42);}
    ...
    else{float fresnel=pow(1.0-noV,5.0),reflectionWeight=clamp(mix(0.10,1.0,fresnel)*uSpecular+surfaceMetalness*0.42+reaction*0.38,0.0,1.0);vec3 reflection=sky(reflect(rd,n));vec3 contactReflection=reflectionField(p,n,reaction,clamp(surfaceMetalness+surfaceTransmission,0.0,1.0));vec3 refraction=sky(refract(rd,n,0.66))*mix(vec3(1.0),albedo,0.20);float highlight=pow(noH,mix(320.0,34.0,surfaceRoughness))*shadow;vec3 dielectric=mix(refraction,reflection+contactReflection,reflectionWeight),metal=(reflection+contactReflection*0.72)*mix(vec3(0.72),albedo,0.72);col=mix(dielectric,metal,surfaceMetalness)+highlight*vec3(1.0,0.98,0.94)*3.2+albedo*(0.035+surfaceEmission*0.72);col=mix(col,refraction+contactReflection*0.22,clamp(surfaceTransmission*(1.0-surfaceMetalness)*0.34,0.0,0.78));col=mix(col,col*0.52+reflection*0.26+contactReflection*0.94,reaction*0.74);col*=mix(0.58,1.0,surfaceAlpha);}
...
function fail(message){ errorPanel.hidden=false; errorMessage.textContent=message; document.getElementById('renderStatus').textContent=t('renderUnavailable'); }
...
let program;
try {
  if(!gl) throw new Error(t('webgl2Unavailable'));
...
function residueVector(object){const residue=object?.residue||{};return [residue.strength||0,residue.memory||0,residue.optical||0,residue.geometry||0];}
function residueColor(object){return object?.residue?.color||[0.72,0.93,1.0];}
function render(now){
  window.mathDomainRouter?.update(state,now/1000);const effective=state.domainModel?.effective||state;
  gl.uniform1f(uniforms.uConsumeScale,effective.consumeScale); gl.uniform1f(uniforms.uBooleanSmooth,state.booleanSmooth); gl.uniform1f(uniforms.uFrontNoise,effective.frontNoise);
...
['blend','contactThreshold','consumeScale','booleanSmooth','frontNoise','dissolveRate','recoveryRate','spacing','radius','boxSize','roughness','specular','transmission','ior'].forEach(id=>{ const input=document.getElementById(id),output=document.querySelector(`output[for=${id}]`); if(!input)return; if(id==='specular')input.max='1.4'; if(state[id]!==undefined){input.value=state[id];if(output)output.value=Number(state[id]).toFixed(2);} input.addEventListener('input',()=>{state[id]=Number(input.value);if(output)output.value=Number(input.value).toFixed(2);}); });
...
const materialPresets={
  marble:{roughness:.025,specular:1.18,transmission:1,ior:1.57,color:[.72,.93,1],swatch:0},
  crystal:{roughness:.018,specular:1.22,transmission:.96,ior:1.72,color:[.92,.97,1],swatch:0},
...
document.querySelectorAll('.material-preset').forEach(button=>button.addEventListener('click',()=>{const preset=materialPresets[button.dataset.material];Object.assign(state,{roughness:preset.roughness,specular:preset.specular,transmission:preset.transmission,ior:preset.ior,color:[...preset.color]});document.querySelectorAll('.material-preset').forEach(item=>item.classList.toggle('active',item===button));document.querySelectorAll('.swatch').forEach((item,index)=>item.classList.toggle('active',index===preset.swatch));document.getElementById('materialStatus').textContent=t(button.dataset.material);syncMaterialControls();}));
document.querySelectorAll('.swatch').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('.material-preset').forEach(item=>item.classList.remove('active'));document.getElementById('materialStatus').textContent=t('customColor');}));
for(const id of ['roughness','specular','transmission','ior'])document.getElementById(id).addEventListener('input',()=>{document.querySelectorAll('.material-preset').forEach(item=>item.classList.remove('active'));document.getElementById('materialStatus').textContent=t('customGlass');});
const presets={blend:{preset:0,blend:.34,spacing:1.0,radius:1,boxSize:.78},stack:{preset:1,blend:.30,spacing:1.24,radius:.82,boxSize:.68},orbit:{preset:2,blend:.22,spacing:1.55,radius:.7,boxSize:.62}};
...
let meshProgram=null,meshFrames=[],meshSdfTexture=null,meshMaterialTexture=null,meshFeaturesTexture=null,meshSdfData=null,meshMaterialData=null,meshFeaturesData=null,webgpuRenderer=null,threeRenderer=null,threeModelReady=false,pendingThreeFile=null,pendingThreeFiles=[],meshVertexCount=0,sequencePlaying=true,sequenceStart=performance.now(),sequenceFrame=0;
function rebuildImportedVolume(){
  ...
  const sdf=new Float32Array(resolution**3),material=new Uint8Array(resolution**3*4),features=new Uint8Array(resolution**3*4);
  for(let z=0;z<resolution;z++)for(let y=0;y<resolution;y++)for(let x=0;x<resolution;x++){
    const p=[x,y,z].map((value,axis)=>center[axis]+((value/(resolution-1))-.5)*bounds[axis]*2),index=x+resolution*(y+resolution*z);let best=Infinity,second=Infinity,winner=null,runnerUp=null,winnerIndex=0,runnerIndex=0;
    for(const item of sources){const local=p.map((value,axis)=>(value-item.position[axis])/item.scale),uv=local.map((value,axis)=>value/(item.sourceBounds[axis]*2)+.5),clamped=uv.map(value=>Math.max(0,Math.min(1,value))),grid=clamped.map(value=>Math.round(value*(item.sdfSize-1))),sourceIndex=grid[0]+item.sdfSize*(grid[1]+item.sdfSize*grid[2]),outside=Math.hypot(...uv.map((value,axis)=>Math.max(Math.abs(value-.5)-.5,0)*item.sourceBounds[axis]*2)),distance=item.sdfData[sourceIndex]*item.scale+outside;if(distance<best){second=best;runnerUp=winner;runnerIndex=winnerIndex;best=distance;winner=item;winnerIndex=sourceIndex;}else if(distance<second){second=distance;runnerUp=item;runnerIndex=sourceIndex;}}
    const k=Math.max(state.blend*.72,.025),h=Number.isFinite(second)?Math.max(k-Math.abs(best-second),0)/k:0;sdf[index]=Number.isFinite(second)?Math.min(best,second)-h*h*k*.25:best;const a=winner?.materialData?.subarray(winnerIndex*4,winnerIndex*4+4)||[143,158,173,72],b=runnerUp?.materialData?.subarray(runnerIndex*4,runnerIndex*4+4)||a,fa=winner?.featuresData?.subarray(winnerIndex*4,winnerIndex*4+4)||[0,255,0,0],fb=runnerUp?.featuresData?.subarray(runnerIndex*4,runnerIndex*4+4)||fa,weight=h*.5;for(let channel=0;channel<4;channel++){material[index*4+channel]=Math.round(a[channel]*(1-weight)+b[channel]*weight);features[index*4+channel]=Math.round(fa[channel]*(1-weight)+fb[channel]*weight);}
  }
  meshSdfData=sdf;meshMaterialData=material;meshFeaturesData=features;state.objects.mesh={...state.objects.mesh,position:center,scale:1,bounds};
  ...
  if(meshFeaturesTexture)gl.deleteTexture(meshFeaturesTexture);meshFeaturesTexture=gl.createTexture();gl.bindTexture(gl.TEXTURE_3D,meshFeaturesTexture);for(const parameter of [gl.TEXTURE_MIN_FILTER,gl.TEXTURE_MAG_FILTER])gl.texParameteri(gl.TEXTURE_3D,parameter,gl.LINEAR);for(const parameter of [gl.TEXTURE_WRAP_S,gl.TEXTURE_WRAP_T,gl.TEXTURE_WRAP_R])gl.texParameteri(gl.TEXTURE_3D,parameter,gl.CLAMP_TO_EDGE);gl.texImage3D(gl.TEXTURE_3D,0,gl.RGBA8,resolution,resolution,resolution,0,gl.RGBA,gl.UNSIGNED_BYTE,features);
  webgpuRenderer?.setVolume(sdf,material,resolution,features);threeRenderer?.setVolume?.(sdf,material,resolution,features);state.meshVolumeReady=true;document.getElementById('fusionLabel').textContent=`${t('trueSdf')} · ${sources.length} ${t('sourceMaterialFields')}`;
}
...
function normalizeMeshSequence(frames){
  let min=[Infinity,Infinity,Infinity],max=[-Infinity,-Infinity,-Infinity];for(const data of frames){if(data.length<18)throw new Error(t('emptyMesh'));for(let i=0;i<data.length;i+=6)for(let k=0;k<3;k++){min[k]=Math.min(min[k],data[i+k]);max[k]=Math.max(max[k],data[i+k]);}}
...
async function importFiles(fileList){
  ...
  if(glbs.length){await importGlbs(glbs.slice(0,5));const importedVolume=state.imported.find(item=>item.file===glbs[0]);if(importedVolume&&meshMaterialData)importedVolume.materialData=meshMaterialData;rebuildImportedVolume();if(glbs.length>5)showToast(t('maxGlb'),true);return;}
  const unsupported=files.find(file=>['abc','vdb'].includes(file.name.split('.').pop().toLowerCase()));if(unsupported){showToast(`${unsupported.name}: ${t('unsupportedServer')}`,true);return;}
  if(files.some(file=>!['obj','stl'].includes(file.name.split('.').pop().toLowerCase()))){showToast(t('chooseMeshFormat'),true);return;}
  try{showToast(tr('parsingFrames',{count:files.length}));const raw=[];for(const file of files){const ext=file.name.split('.').pop().toLowerCase();raw.push(ext==='obj'?parseObj(await file.text()):parseStl(await file.arrayBuffer()));}const normalized=normalizeMeshSequence(raw);
    ...
    const label=files.length>1?`${files[0].name} +${files.length-1}`:files[0].name;document.getElementById('importedObjects').innerHTML=`<button class="scene-item" data-object="mesh"><span class="shape-icon mesh"></span><span><strong>${label.replace(/[<>&]/g,'')}</strong><small>${files.length} ${t(files.length>1?'frames':'frame')} · ${t('meshCache')}</small></span><span class="visibility">●</span></button>`;bindSceneItems();selectObject('mesh');showToast(tr('importedFrames',{name:label,count:files.length}));
  }catch(error){showToast(`${t('importFailed')}: ${error.message}`,true);}
}
async function importGlbs(files){
  ...
  if(!accepted.length){showToast(t('reachedLimit'),true);return;}
  ...
  state.imported=allFiles.map((item,index)=>{const previous=existing[index];return{id:`mesh-${index}`,name:item.name,file:item,position:previous?.position||[(index%3-1)*1.35,Math.floor(index/3)*0.95-0.45,0],scale:previous?.scale||1,bounds:previous?.bounds||[1,1,1],role:previous?.role||'real',residue:previous?.residue||{strength:0,memory:0,optical:0,geometry:0,color:[0.72,0.93,1]}};});
  ...
  if(threeRenderer){threeRenderer.loadFile(file).then(()=>{threeModelReady=true;updateMaterialReadout();document.getElementById('renderStatus').textContent=webgpuRenderer?'Three.js PBR + WebGPU SDF':'Three.js PBR + SDF';showToast(tr('threeParsed',{count:state.imported.length}));}).catch(error=>{const detail=String(error?.stack||error?.message||error);showToast(`${t('threeLoadFailed')}: ${error.message}`,true);fetch(`/client-error?source=three-load&message=${encodeURIComponent(detail)}`).catch(()=>{});});}
  try{
    showToast(`${t('blenderConverting')} ${file.name}`);document.getElementById('renderStatus').textContent='GLB';
    const form=new FormData();form.append('file',file);const response=await fetch('/api/convert-glb',{method:'POST',body:form});const manifest=await response.json();if(!response.ok)throw new Error(manifest.error||t('importFailed'));
    const converted=[];for(const url of manifest.frames){const frameResponse=await fetch(url);if(!frameResponse.ok)throw new Error(t('importFailed'));converted.push(new File([await frameResponse.blob()],url.split('/').pop(),{type:'model/stl'}));}
    ...
    if(manifest.sdf){
      ...
      document.getElementById('fusionLabel').textContent=t('trueSdfMaterial');showToast(t('trueSdfEnabled'));
      try{const materialResponse=await fetch(resolveCacheUrl(manifest.sdf.materialUrl));if(!materialResponse.ok)throw new Error();const materials=new Uint8Array(await materialResponse.arrayBuffer());if(materials.length!==size*size*size*4)throw new Error();meshMaterialData=materials;meshMaterialTexture=gl.createTexture();gl.bindTexture(gl.TEXTURE_3D,meshMaterialTexture);gl.texParameteri(gl.TEXTURE_3D,gl.TEXTURE_MIN_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_3D,gl.TEXTURE_MAG_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_3D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_3D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_3D,gl.TEXTURE_WRAP_R,gl.CLAMP_TO_EDGE);gl.texImage3D(gl.TEXTURE_3D,0,gl.RGBA8,size,size,size,0,gl.RGBA,gl.UNSIGNED_BYTE,materials);document.getElementById('fusionLabel').textContent=t('trueSdfFeature');}catch{meshMaterialData=new Uint8Array(size*size*size*4);for(let i=0;i<meshMaterialData.length;i+=4){meshMaterialData[i]=143;meshMaterialData[i+1]=158;meshMaterialData[i+2]=173;meshMaterialData[i+3]=72;}showToast(t('metalFallback'),true);}
      try{const featuresUrl=manifest.sdf.materialFeaturesUrl;if(!featuresUrl)throw new Error();const featuresResponse=await fetch(resolveCacheUrl(featuresUrl));if(!featuresResponse.ok)throw new Error();const features=new Uint8Array(await featuresResponse.arrayBuffer());if(features.length!==size*size*size*4)throw new Error();meshFeaturesData=features;meshFeaturesTexture=gl.createTexture();gl.bindTexture(gl.TEXTURE_3D,meshFeaturesTexture);gl.texParameteri(gl.TEXTURE_3D,gl.TEXTURE_MIN_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_3D,gl.TEXTURE_MAG_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_3D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_3D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_3D,gl.TEXTURE_WRAP_R,gl.CLAMP_TO_EDGE);gl.texImage3D(gl.TEXTURE_3D,0,gl.RGBA8,size,size,size,0,gl.RGBA,gl.UNSIGNED_BYTE,features);const volumeTarget=state.imported.find(item=>item.file===file);if(volumeTarget){volumeTarget.materialData=meshMaterialData;volumeTarget.featuresData=features;}document.getElementById('fusionLabel').textContent=t('trueSdfMaterial');}catch{meshFeaturesData=new Uint8Array(size*size*size*4);for(let i=0;i<meshFeaturesData.length;i+=4)meshFeaturesData[i+1]=255;}
      rebuildImportedVolume();if(webgpuRenderer)webgpuRenderer.setVolume(meshSdfData,meshMaterialData,size,meshFeaturesData);threeRenderer?.setVolume?.(meshSdfData,meshMaterialData,size,meshFeaturesData);state.meshVolumeReady=true;fusion.checked=true;fusion.disabled=false;setFusionMode(true);
    }
    document.getElementById('importedObjects').querySelector('strong').textContent=file.name.replace(/[<>&]/g,'');document.getElementById('importedObjects').querySelector('small').textContent=`${manifest.frames.length} ${t(manifest.frames.length>1?'frames':'frame')} · GLB/Blender`;showToast(tr('importedFrames',{name:'GLB',count:manifest.frames.length}));
  }catch(error){fusion.disabled=!meshSdfTexture;showToast(`${t('glbImportFailed')||t('importFailed')}: ${error.message}`,true);}finally{document.getElementById('renderStatus').textContent=window.unifiedRendererActive?t('threeUnified'):(threeModelReady?'Three.js PBR + SDF':(webgpuRenderer?t('webgpuRealtime'):t('webglFallback')));}
}
...
window.addEventListener('webgpu-lost',event=>{webgpuRenderer=null;document.getElementById('viewport').classList.remove('webgpu-active');const status=document.getElementById('renderStatus');status.textContent=`${t('webglFallback')} · ${t('webgpuDeviceLost')}`;status.title=event.detail||'';showToast(`${t('webgpuDeviceLost')}: ${event.detail||''}`,true);});
if(window.createWebGpuRenderer){window.createWebGpuRenderer(document.getElementById('gpuCanvas'),()=>state).then(renderer=>{if(!renderer){document.getElementById('renderStatus').textContent=`${t('webglFallback')} · ${t('noWebgpuAdapter')}`;return;}if(window.unifiedRendererActive){renderer.stop();return;}webgpuRenderer=renderer;if(meshSdfData&&meshMaterialData)renderer.setVolume(meshSdfData,meshMaterialData,Math.round(Math.cbrt(meshSdfData.length)),meshFeaturesData);document.getElementById('viewport').classList.add('webgpu-active');document.getElementById('renderStatus').textContent=threeModelReady?'Three.js PBR + WebGPU SDF':t('webgpuRealtime');}).catch(error=>{console.warn('WebGPU fallback:',error);const message=String(error?.message||error),status=document.getElementById('renderStatus');status.textContent=`${t('webglFallback')} · ${message.slice(0,72)}`;status.title=message;showToast(`${t('webgpuError')}: ${message}`,true);fetch(`/client-error?source=webgpu&message=${encodeURIComponent(message)}`).catch(()=>{});});}
async function initializeThree(){if(!window.createThreeRenderer||threeRenderer)return;try{threeRenderer=await window.createThreeRenderer(document.getElementById('threeCanvas'),()=>state);if(meshSdfData&&meshMaterialData)threeRenderer.setVolume(meshSdfData,meshMaterialData,Math.round(Math.cbrt(meshSdfData.length)),meshFeaturesData);if(pendingThreeFile){await threeRenderer.loadFile(pendingThreeFile);threeModelReady=true;updateMaterialReadout();document.getElementById('renderStatus').textContent=t('threeUnified');}}catch(error){console.warn('Three.js fallback:',error);const detail=String(error?.stack||error?.message||error);showToast(`${t('threeInitFailed')}: ${error.message}`,true);fetch(`/client-error?source=three-init&message=${encodeURIComponent(detail)}`).catch(()=>{});}}
